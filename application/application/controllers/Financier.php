...
#======================================================================================
    public function code_validation(){
        ...
        $al = "0123456789abcdefghijklmnopqrstuvwxyz#*+ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        $code_validation = substr(str_shuffle(str_repeat($al, rand(5, 20))), 0, 10);
    ...
    function completer_paiement ()
    {
        ...
        $date_paiement = $this->input->post('date_paiement');
        $date_validation = date('Y-m-d');
        $montant_percu = $this->input->post('montant_restant');
        ...
        $paiements_eleve = $this->School_management->get_join(['tb_school_paiements', 'inscriptions'], ['matricule_eleve'],
            ['paiements.annee_scolaire' => $annee_scolaire, 'paiements.matricule_eleve' => $matricule_eleve],
            'paiements.*', 'paiements.id', 'ASC');

        if ($devise == $frais->devise){
            $montant_complet = $montant_percu . " ". $devise;
