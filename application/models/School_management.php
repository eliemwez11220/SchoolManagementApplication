...
class School_management extends CI_Model
{
    ...
    public function view_paiement_eleve($code_paiement = null, $matricule = null, $limit = 1)
    {
        if (!empty($limit)) {
            $this->db->limit($limit);
        }
        
        $this->db->order_by('tb_school_paiements.date_paiement', 'DESC');
        
        $this->db->join('tb_school_eleves', 'tb_school_eleves.matricule_eleve = tb_school_paiements.matricule_eleve');
        //$this->db->join('tb_school_frais', 'tb_school_frais.type_frais = tb_school_paiements.type_frais');
        //$this->db->join('tb_school_eleves', 'tb_school_eleves.matricule_eleve = tb_school_inscriptions.matricule_eleve');
        if ($code_paiement) {
            $query = $this->db->get_where('tb_school_paiements', array('code_validation' => $code_paiement))->row_array();
            return $query;
            //return $this->db->get_where('tb_pm_visiteurs', array('id_visiteur' => $visiteur_id))->row_array();
        } else {
            $query = $this->db->get('tb_school_paiements')->result();
            return $query;
        }
    }
public function view_inscription_eleve($matricule = null, $id = null, $limit = null)
    {
        if (!empty($limit)) {
            $this->db->limit($limit);
        }
        
        $this->db->order_by('tb_school_inscriptions.date_inscription', 'DESC');
        $this->db->join('tb_school_eleves', 'tb_school_eleves.matricule_eleve = tb_school_inscriptions.matricule_eleve');
       // $this->db->join('tb_school_eleves', 'tb_school_eleves.matricule_eleve = tb_school_paiements.matricule_eleve');
        $this->db->join('tb_school_frais', 'tb_school_frais.type_frais = tb_school_inscriptions.type_frais');
         $this->db->join('tb_school_classes', 'tb_school_classes.id_classe = tb_school_inscriptions.code_classe');
        if ($id) {
            $query = $this->db->get_where('tb_school_inscriptions', array('id' => $id))->row_array();
            return $query;
        } else {
            $query = $this->db->get('tb_school_inscriptions')->result();
            return $query;
        }
    }
